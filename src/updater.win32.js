...
if (process.platform === 'win32') { // Should be the only possible case, but we never know
  ...
  if (buildInfo && buildInfo.newUpdater) {
    ...
    require.cache[autoStartScript].exports.update = async (callback) => {
      ...
      console.log('[Vibecord] Checking for host updates...');

      if (!existsSync(appDir)) {
        console.log('[Vibecord] Host update is available! Injecting into new version...');
        return inject(injector).then(() => {
          console.log('[Vibecord] Successfully injected into new version!');

          update(callback);
        });
      }

      console.log(`[Vibecord] Host "${buildInfo.version}" is already injected with Vibecord.`);
    };
  } else {
